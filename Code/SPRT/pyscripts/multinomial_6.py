################################################################################
# SLURM Array Job 6
# SPRT of sample probabilities (multinomial equal p), SHA256, n=13, k=3, s = 10, sample_by_index
################################################################################
...
def testSeed(ss, n, k, s):
    ...
    return ["SHA256", "sample_by_index", ss, res['decision'], res['LR'][-1], res['pvalue'], res['steps'], n, k, s]
...
with open('../rawdata/SHA256_multinomial_sbi.csv', 'at') as csv_file:

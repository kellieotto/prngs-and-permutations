...
def testSeed(ss):
    ...
    prng = SHA256(ss)
    val = np.array(serial_perm_distr(500, reps=2*10**2, gen=prng), dtype=int)
...
#result = list(map(wrapper, range(len(seed_values[:5]))))
result = lview.map(lambda ss: testSeed(ss), seed_values[:5])

# Write results to file
with open("sha_fp_distr_500.csv", "w") as f:
    w = csv.writer(f)
    for pp in range(len(pvalues[:5])):

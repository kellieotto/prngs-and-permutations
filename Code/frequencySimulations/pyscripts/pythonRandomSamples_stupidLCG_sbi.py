# Stupid Counter LCG: X_n+1 = (X_n + 1) mod 2**32
...
# Parameters for stupid counter LCG
A_counter = 1
B_counter = 1
M_counter = 2**10
...
for nn in n:
    for kk in k:
        ...
        for ss in seedvalues:
            counter_lcg = lcgRandom(seed=ss, A=A_counter, B=B_counter, M=M_counter)            
            uniqueSampleCounts = None

            for rr in range(len(reps)):
                uniqueSampleCounts = getEmpiricalDistr(counter_lcg, sample_by_index, n=nn, k=kk, reps=rep_diffs[rr], uniqueSamples=uniqueSampleCounts)
                ...
                with open('../rawdata/US_stupidLCG_sbi.csv', 'at') as csv_file:
                    writer = csv.writer(csv_file)
                    writer.writerow([nn, kk, ss, reps[rr], "sample_by_index", "counter_lcg", 
